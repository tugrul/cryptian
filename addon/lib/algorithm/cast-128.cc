...
std::string Cast128::getName() {
    return "CAST-128";
}

std::size_t Cast128::getVersion() {
    return 20010801;
}

std::size_t Cast128::getBlockSize() {
    return 8;
}

std::vector<std::size_t> Cast128::getKeySizes() {
    return {16};
}

std::vector<char> Cast128::encrypt(const std::vector<char> plaintext) {
    ...
    std::copy_n(plaintext.begin(), 8, block);

    // Get inblock into l,r
    l = ((unsigned int) block[0] << 24) | ((unsigned int) block[1] << 16)
        | ((unsigned int) block[2] << 8) | (unsigned int) block[3];
    r = ((unsigned int) block[4] << 24) | ((unsigned int) block[5] << 16)
        | ((unsigned int) block[6] << 8) | (unsigned int) block[7];
    ...
    return std::vector<char>(block, block + 8);
}

std::vector<char> Cast128::decrypt(const std::vector<char> ciphertext) {
    ...
    std::copy_n(ciphertext.begin(), 8, block);

	// Get inblock into l,r
	r = ((unsigned int) block[0] << 24) | ((unsigned int) block[1] << 16)
	    | ((unsigned int) block[2] << 8) | (unsigned int) block[3];
	l = ((unsigned int) block[4] << 24) | ((unsigned int) block[5] << 16)
	    | ((unsigned int) block[6] << 8) | (unsigned int) block[7];
...
void Cast128::reset() {
    ...
    unsigned int t[4], z[4], x[4];
	unsigned int i;

	// Set number of rounds to 12 or 16, depending on key length
	rounds = (_key.size() <= CAST_SMALL_KEY) ? CAST_SMALL_ROUNDS : CAST_FULL_ROUNDS;
