...
std::string Blowfish::getName() {
    return "Blowfish";
}

std::size_t Blowfish::getVersion() {
    return 20010801;
}

std::size_t Blowfish::getBlockSize() {
    return 8;
}

std::vector<std::size_t> Blowfish::getKeySizes() {
    return {56};
}
...
std::vector<char> Blowfish::encrypt(const std::vector<char> plaintext) {
	...
	for (size_t i = 0; i < BF_N; ++i) {
        ...
        std::swap(ciphertext.ui[0], ciphertext.ui[1]);
	}

	std::swap(ciphertext.ui[0], ciphertext.ui[1]);

	ciphertext.ui[1] ^= P[BF_N];
	ciphertext.ui[0] ^= P[BF_N + 1];
...
std::vector<char> Blowfish::decrypt(const std::vector<char> ciphertext) {
	...
	for (size_t i = BF_N + 1; i > 1; --i) {
		...
		std::swap(plaintext.ui[0], plaintext.ui[1]);
	}
...
void Blowfish::enblf_noswap(unsigned int *x)
{
	unsigned int Xl, Xr;
	short i;
...
const unsigned int Blowfish::pi[18] = {
...
const unsigned int Blowfish::ks[4][256] = {{
